...
sub main{
  ...
  GetOptions($settings,qw(help makeTree=i prefix=s numcpus=i tempdir=s allowed|allowedFlanking=i));
  $$settings{prefix}||="./out";
  $$settings{numcpus}||=1;
  ...
  # Check if user requested to build tree
  if($$settings{makeTree} == 1){
    ...
    logmsg "Skipping tree construction as requested.";
  }
