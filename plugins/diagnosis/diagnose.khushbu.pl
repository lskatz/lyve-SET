...
sub main{
  ...
  foreach my $bam(glob("$bam_path/*.bam")){
    ...
    my $i=$bam;
               ...
               $perc_mapped_reads = ($total_mapped_reads/$tot_reads) * 100; #Calculating percentage of mapped reads
               $perc_mapped_reads_gt_30 = ($mapped_reads/$tot_reads) * 100; #Calculating percentage of mapped reads > Q30
